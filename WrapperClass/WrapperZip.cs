...
namespace nsWrapperZip
{
    //based on - http://dotnetzip.codeplex.com/wikipage?title=CS-Examples&referringTitle=Examples
    // DotNetZip
    ...
    public static class WrapperZip
    {
            ...
            using (ZipFile zip = ZipFile.Read(strPathZipFile))
            {
                try
                {
                    if (File.Exists(strPathZipFile) == true)
                    {
                        ...
                        {
                            WrapperUnion.WrapperFile.CreateFolder(strDestination);
                        }
            ...
            try
            {
                using (ZipFile zip = new ZipFile())
                {
                    for (int i = 0; i < listFiles.Count; i++)
                    {
                        ...
                        if (WrapperUnion.WrapperFile.isValidPath(strFilePath) == true)
                        {
        ...
        public static Boolean CreateZipDirectory(string strDirectory, string strZipFilePath)
        {
            ...
            try
            {
                using (ZipFile zip = new ZipFile())
                {
                    string [] files = WrapperUnion.WrapperFile.GetFileList_within_subdir(strDirectory);
